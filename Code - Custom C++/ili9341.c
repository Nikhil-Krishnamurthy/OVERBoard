...
void LCD_setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
    ...
    // Create byte arrays for SPI transmission (Big-Endian split)
    uint8_t xa[4] = {
        (uint8_t)(x >> 8),   // Column Start MSB
        (uint8_t)(x & 0xFF),  // Column Start LSB
        (uint8_t)(x_end >> 8), // Column End MSB
        (uint8_t)(x_end & 0xFF)// Column End LSB
    };

    uint8_t ya[4] = {
        (uint8_t)(y >> 8),   // Row Start MSB
        (uint8_t)(y & 0xFF),  // Row Start LSB
        (uint8_t)(y_end >> 8), // Row End MSB
        (uint8_t)(y_end & 0xFF)// Row End LSB
    };
